//old dictionary. Will still be used if the new dictionary fails to load for some reason
...
//new dictionary. All words pulled from the text document listed
...
function randomWordJson(currMode) {
   ...
   //        the 'Hangman word';
       ...
       console.log("Score: " + getWordScore(word));
       console.log(obj);
       return obj;
...
function getWordScore(word) {
	var sum = 0;
	for (var i = 0; i < word.length; i++) {
		sum += getLetterScore(word[i],word.length);
	}
	return sum;
}

//returns the letter score, which is based on the Scrabble value of the letter and the length of the word.
